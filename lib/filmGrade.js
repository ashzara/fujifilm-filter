...
// Film tone curve — asymmetric S: shadows fall deeper, highlights stay open.
// The extra toe pull below 35% L* gives the "rich dark" film quality.
function softCurve(L, strength) {
  const x = L / 100;
  const s   = x + strength * 0.18 * x * (1 - x) * (2 * x - 1);  // standard S
  const toe = strength * 0.09 * -Math.pow(Math.max(0, 0.38 - x), 2); // shadow toe
  return Math.max(0, Math.min(100, (s + toe) * 100));
}
...
function getHueChromaScale(hDeg) {
  ...
  // Pulled back toward 1.0 across the board — the "muted" quality
  // should come from the lifted blacks + soft curve, not chroma compression.
  // Only greens get a meaningful pull (Classic Chrome's signature).
  if (d < 25  || d >= 340) return 0.96; // reds/magentas: barely touched
  if (d < 55)              return 0.99; // red-orange / skin ← nearly untouched
  if (d < 80)              return 0.97; // oranges / warm yellow
  if (d < 110)             return 0.93; // yellows → yellow-greens
  if (d < 165)             return 0.88; // greens  ← Classic Chrome suppresses these
  if (d < 205)             return 0.94; // teals / cyans
  if (d < 255)             return 0.99; // blues   ← sky stays rich
  if (d < 295)             return 0.95; // blue-violets
  return 0.94;                          // purples
}
...
const CONFIGS = {
  flash: {
    blackLift: 0,  contrast: 0.72,
    hA: -0.3, hB: -0.5,
    mA:  0.9, mB:  0.5,   // more midtone warmth → richer skin
    sA: -0.4, sB: -0.7,
    grain: 7,
  },
  lowlight: {
    blackLift: 4,  contrast: 0.65,
    hA: -0.5, hB: -0.9,
    mA:  0.7, mB:  0.4,
    sA: -0.5, sB: -0.9,
    grain: 10,
  ...
  daylight: {
    blackLift: 5,  contrast: 0.75,  // more contrast = depth without HDR
    hA: -0.3, hB: -0.4,
    mA:  0.9, mB:  0.5,   // warm, present skin tones
    sA: -0.3, sB: -0.5,
    grain: 4,
  },
  overexposed: {
    blackLift: 4,  contrast: 0.70,
    hA: -0.3, hB: -0.4,
    mA:  0.8, mB:  0.4,
    sA: -0.3, sB: -0.6,
    grain: 5,
