...
// Film tone curve — asymmetric S: shadows fall noticeably deeper, highlights stay open.
function softCurve(L, strength) {
  const x = L / 100;
  const s   = x + strength * 0.18 * x * (1 - x) * (2 * x - 1);   // standard S
  const toe = strength * 0.14 * -Math.pow(Math.max(0, 0.42 - x), 2); // deeper shadow toe
...
function getHueChromaScale(hDeg) {
  ...
  if (d < 25  || d >= 340) return 0.95; // reds/magentas
  if (d < 55)              return 0.99; // red-orange / skin ← nearly untouched
  if (d < 80)              return 0.96; // oranges / warm yellow
  if (d < 110)             return 0.88; // yellows → yellow-greens: more muted
  if (d < 165)             return 0.81; // greens ← pushed harder, olive/muted like film
  if (d < 205)             return 0.90; // teals / cyans
  if (d < 255)             return 0.98; // blues ← sky stays rich
  if (d < 295)             return 0.93; // blue-violets
  return 0.93;                          // purples
}
...
const CONFIGS = {
  ...
  daylight: {
    blackLift: 3,  contrast: 0.80,  // less lift + more curve = richer shadows
    hA: -0.3, hB: -0.4,
    mA:  0.9, mB:  0.5,
    sA: -0.3, sB: -0.5,
    grain: 4,
